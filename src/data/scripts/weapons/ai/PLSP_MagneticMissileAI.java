...
public class PLSP_MagneticMissileAI implements MissileAIPlugin, GuidedMissileAI {
	...
	private float mirvCooldown = 1f;
	private final float accThreshold = 7.5f;

	//////////////////////
	//  DATA COLLECTING //
	//////////////////////
	public PLSP_MagneticMissileAI(MissileAPI missile, ShipAPI launchingShip) {
	...
	public void advance(float amount) {
		...
		if (alwaysAcc && MathUtils.getDistance(target, missile) <= 450f && mirvCooldown <= 0f) {
			if (mirvAmount == 0) {
				mirv(missile);
				mirvCooldown = 0.75f;
				mirvAmount++;
	...
	private void mirv(MissileAPI missile) {
		Vector2f spawnPoint = MathUtils.getPoint(missile.getLocation(), 5f, missile.getFacing());
		float angle = missile.getFacing() + (float)Math.random() * 10f - (float)Math.random() * 10f;
		...
		float vel = 0.8f + (float)Math.random() * 0.4f;
		...
		for (int i2 = 0; i2 < 2; i2++) {
			...
			float vel2 = 0.6f + (float)Math.random() * 1f;
			projectile2.getVelocity().scale(vel2);
		}

		Global.getSoundPlayer().playSound("PLSP_magnetics_burst", 1f, 1f, missile.getLocation(), zero);
		MagicLensFlare.createSharpFlare(Global.getCombatEngine(), missile.getSource(), spawnPoint, 3f, 120f, 20f, PLSP_ColorData.BRIGHT_EMP_ARC_FRINGE, PLSP_ColorData.BRIGHT_EMP_ARC_CORE);
	}
